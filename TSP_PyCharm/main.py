...
def rilassamento_continuo_vs_pa():
    costs_matrices = get_test_cost_matrices()

    for i, cm in enumerate(costs_matrices):
        print(f"Id {i + 1}")
        print("PA: ")
        solve_with_cut_set(cm, connected=False, visualize=False, print_time=True)
        print("MTZ")
        solve_with_MTZ(cm, visualize=False, print_time=True, relaxated=True)
...
if __name__ == '__main__':
    ...
    # solve_with_MTZ(costs_matrix, visualize=False, print_time=True, relaxated=False)

    # bb_total_vs_binary()
    # test_mtz_time()

    rilassamento_continuo_vs_pa()
